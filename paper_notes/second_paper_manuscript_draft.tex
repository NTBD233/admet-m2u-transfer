...
All neural students use ECFP4 fingerprints with radius 2 and 2048 bits as the
only inference-time molecular input. The descriptor reconstruction branch uses
nine RDKit descriptors: molecular weight, LogP, TPSA, hydrogen-bond acceptors,
hydrogen-bond donors, rotatable bonds, aromatic rings, heavy atoms, and ring
count. Students are trained with Adam using a learning rate of \(5\times
10^{-4}\), weight decay \(10^{-5}\), batch size 64, at most 200 epochs, and
early stopping with patience 30 on validation RMSE. Unless otherwise stated,
the descriptor reconstruction weight is 0.1 and the multi-teacher distillation
weight is 1.0.

The RF teachers are trained on the same labeled training split as the student
and differ only in molecular view: \texttt{ECFP4\_RF}, \texttt{Desc\_RF}, and
\texttt{ECFP4\_Desc\_RF}. Each RF regressor uses 500 trees and square-root
feature subsampling. The formal selector is an RF classifier trained from
cross-fit pseudo-oracle labels. Its input contains per-teacher uncertainty,
per-teacher consensus gap, per-teacher validation prior weight, and a shared
descriptor-space OOD distance. After selector pretraining, selector parameters
are frozen during student distillation.

We enforce leakage control at the label-construction stage. For every
dataset-ratio-seed setting, selector labels are generated inside the training
split using \(K\)-fold out-of-fold teacher predictions. Thus, a training sample's
pseudo-oracle teacher is determined only by teachers that did not fit that
sample in the corresponding fold. Validation and test examples are not used to
construct selector supervision; validation predictions are used only for model
selection, teacher-prior estimation, and validation-selected baselines.
The experiment pipeline is organized as Makefile targets for selector training,
frozen-routing student training, high-resource calibration, and manuscript table
regeneration from saved metrics.
...
\bottomrule
\end{tabular}
\caption{Full 45-run regression comparison. Lower RMSE is better. Deltas are
...
\subsection{Extension-panel robustness}

To test whether the five-endpoint result is narrowly tied to the original
benchmark panel, we also ran a four-endpoint regression extension panel:
\texttt{clearance\_hepatocyte\_az}, \texttt{clearance\_microsome\_az},
\texttt{half\_life\_obach}, and \texttt{ld50\_zhu}. This panel was not used to
define the main method or tune selector routing; we use it as a robustness
check for the calibrated claim.
...
\toprule
Method & RMSE & \(\Delta\) Base & Wins & \(\Delta\) Fixed & Wins & \(\Delta\) Top1 & Wins \\
\midrule
Base AdapterFusion & 27.8369 & -- & -- & 0.0226 & 14/36 & 0.4880 & 9/36 \\
Fixed ECFP4+Desc RF & 27.8143 & -0.0226 & 22/36 & -- & -- & 0.4654 & 9/36 \\
Top-1 validation & 27.3490 & -0.4880 & 27/36 & -0.4654 & 27/36 & -- & -- \\
Pretrained selector & 27.4158 & -0.4211 & 26/36 & -0.3985 & 25/36 & 0.0668 & 16/36 \\
Selector + high-resource decay & 27.4865 & -0.3504 & 26/36 & -0.3278 & 27/36 & 0.1376 & 16/36 \\
\bottomrule
\end{tabular}
\caption{Four-endpoint extension-panel robustness check. Lower RMSE is better.
Deltas are method minus reference. The extension panel contains 36
dataset-ratio-seed runs.}
\label{tab:extension_robustness}
\end{table}

Table~\ref{tab:extension_robustness} supports the bounded version of the
claim. Pretrained selector routing improves over base AdapterFusion and fixed
single-teacher distillation on mean RMSE, and it wins 26/36 and 25/36
seed-level runs against those references, respectively. However,
\texttt{top1\_validation} remains the strongest extension-panel baseline on
mean RMSE, and the selector wins only 16/36 seed-level runs against it. The
high-resource decay schedule is also weaker on this extension panel. We
therefore keep the manuscript claim focused: supervised teacher selection
improves ECFP-only distillation over base and fixed-teacher training, while
validation-level teacher choice remains a strong baseline that sample-level
routing does not uniformly dominate.
...
\subsection{Selector routing patterns}

Figure~\ref{fig:route_mix} examines the teacher choices made by the frozen
selector on the test split. The selector does not collapse to one universal
teacher. \texttt{caco2\_wang} and \texttt{solubility\_aqsoldb} route most
samples to \texttt{Desc\_RF}, consistent with the value of physicochemical
descriptor knowledge for permeability and solubility. In contrast,
\texttt{vdss\_lombardo} at 10\% training routes 55.6\% of test samples to
\texttt{ECFP4\_RF}, while \texttt{lipophilicity\_astrazeneca} and
\texttt{ppbr\_az} show more mixed routing as the train ratio increases. This
supports the central premise that teacher choice is endpoint- and
data-regime-dependent rather than a fixed preference for the descriptor-access
teacher.

We also tested whether selector wins over \texttt{top1\_validation} are simply
explained by larger teacher conflict. This pattern is not supported by the
current full matrix: plain-selector wins have slightly lower mean teacher
prediction range than losses (1.9960 vs. 2.2021). The stronger interpretation is
therefore more specific: sample-level selection changes the teacher policy
within a setting, but downstream gains still depend on whether the student can
use the selected supervision.

\begin{figure}[t]
\centering
\includegraphics[width=0.90\linewidth]{../paper_figures_second/fig5_selector_route_mix.pdf}
\caption{Selector routing choices by endpoint and train ratio. Bars show the
test-split proportion of samples routed to each RF teacher, averaged across
three seeds. \(H\) denotes normalized entropy of the routing distribution.}
\label{fig:route_mix}
\end{figure}
...
reweighting was reproducible and locally useful in specific failure cases, but
it was not robust enough to become a main method. A validation-selected policy
that chose among no reweighting, global confidence reweighting, and
disagreement-only reweighting by validation RMSE improved mean validation RMSE
relative to the plain selector (9.6038 vs. 9.6779), but did not improve mean
test RMSE (8.6244 vs. 8.6091). The earlier auto-reweight rule was weaker still,
increasing mean test RMSE to 8.7875 on the same subset.
...
A direct student-side sanity check supports this diagnosis but also explains
why reweighting is not promoted to the main method. On the same
\texttt{ppbr\_az} setting, global confidence reweighting reduces the selector
gap to top-1 validation from +0.7527 to +0.3227 test RMSE and turns the
sample-level mean absolute-error delta negative. On \texttt{caco2\_wang},
disagreement-only reweighting improves the smoke test RMSE from 1.7337 to
1.6957. However, selecting a reweight mode by validation RMSE across the
18-run \texttt{caco2\_wang} + \texttt{ppbr\_az} subset does not improve test
RMSE over the plain selector. Thus, student-side utilization is a real
bottleneck, but low-resource validation is not yet sufficient for a stable
reweight selection policy.
