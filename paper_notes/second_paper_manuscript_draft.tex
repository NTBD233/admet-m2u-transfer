...
regression ADMET. However, fixed distillation strength is not uniformly
...
uniformly. Instead, the student uses a learned teacher selector
\[
\pi(x)=h_\phi(r(x)),
\]
...
descriptor-space OOD distance is appended to the feature vector. Teacher-student
...
used only as a diagnostic baseline for selector predictability.
...
\texttt{ppbr\_az}. For each endpoint, we use 10\%, 20\%, and 50\% training
